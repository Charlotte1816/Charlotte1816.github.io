...
* **意図問題**(彼の~という発言の趣旨は？という内容の設問)は，その問題を**優先して先読み**しておく．また該当の**発言の後**を注意して聞く．
...
* **文意より型**に注目．例外的に文脈を理解する必要がある．

* 例外として，**複合名詞**や**多義語**は知識がないと解けない．
...
* 「**each ~**」，「**every ~**」，「**either ~**」は主語として**単数形**(いわゆる「3つのe」)．

* 「**every**」，「**almost**」，「**no**」は**単体で主語にならない**．

* **名詞句のあとの所有代名詞**(mineとか)は**強調**の意味．

* **接続詞のあとはSV，前置詞のあとは名詞**．
    * よく出る接続詞：while(のあいだ), because(なぜなら), although(けれども), so that(のために；よって), as if(のように), even though(たとえ何々でも), provided that(もし何々ならば), **since(なので)**
    * よく出る前置詞：during(のあいだ), due to(のせいで), despite(にも拘わらず), past(をこえて), thoughout(ずっと), **since(以来)**, because of(原因で)

* **関係詞は原則，あとに形容詞**．「**what**」だけあとに**名詞**．

* 関係詞「**that**」は**カンマのあとに使わない**．「**which**」は**人でも可**．

* 「**than**」を伴うなら，**比較級**(~er, the ~er, more ~)．

* **分詞構文**は，「そして何々だ」程度の補足説明だと思ってよい．

### Part 6(長文穴埋め問題)

* 上から**普通に読む**．4つ穴があって，3つは文法問題で**文脈に依存しない場合**Part 5と同様(文意より型)．**文脈に依存する場合**(特に時制の問題)と残りの文を入れる問題は，全体から判断する．

* 時制の問題では，「**must**」，「**can**」，「**may**」が未来を表しうるということに注意する．

### Part 7(読解問題)

* Part 6と同様に**普通に読む**．文を入れる問題も全体像を把握した後で良い．
